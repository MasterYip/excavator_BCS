...
//是否使用软件SPI(General函数通过此判断使用软件/硬件)
...
struct IMU_data{
	...
	/* Quaternion ---------------------------------------------------------*/
	float Quaternion[4];
	float QuaRevision[4];//修正四元数（姿态解算使用 RobotArmResolver）
	float LinkQuaternion[4];//修正后的四元数（机械臂定义坐标架的变化四元数）
	float ErrorIntegrate[3];//mahony filter使用
	
	/* Euler Angles ---------------------------------------------------------*/
	//提供给ANO Client
	float EulerAngles[3];//IMU本身的姿态 0-roll(绕x轴) 1-pitch 2-yaw (deg)
	float RevisedEuler[3];//四元数修正后的欧拉角（臂体姿态）
